...
RUN /bin/bash -c "if [[ $BUILD == 'prod' ]] ; then echo \"Production Build\" && cd fastai && "\
                  "pip install . && cd ../fastcore && pip install . && cd ../fastrl && pip install .; fi"
RUN /bin/bash -c "if [[ $BUILD == 'dev' ]] ; then echo \"Development Build\" && cd fastai && pip install -e \".[dev]\""\
                 " && cd ../fastcore && pip install -e \".[dev]\" cd ../fastrl && pip install -e \".[dev]\"; fi"
